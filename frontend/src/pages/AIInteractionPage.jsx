...
const AIInteractionPage = () => {
    ...
    // function to handle backend api
    const handleSendMessageToBackend = async (message) => {
        try {
            const response = await fetch("http://localhost:8000/api/chat", {  // ??URL
                method: "POST",
    ...
    const handleSendMessage = () => {
        ...
        const reply = handleSendMessageToBackend(userInput);
        setMessages((prev) => [...prev, { sender: "ai", text: reply }]);

        playVideo(); // Play video on every message
